...
function catalogFailurePool(component) {
  return {
    async query(sql) {
      ...
      if (component === "schema-version-query" && sql.includes("FROM public.pocket_sync_schema")) throw new Error(SENTINEL);
      if (sql.includes("information_schema.columns")) return { rows: validColumns(), rowCount: 12 };
      if (sql.includes("FROM pg_constraint")) return { rows: validConstraints(), rowCount: 16 };
      if (sql.includes("FROM public.pocket_sync_schema")) return { rows: [
