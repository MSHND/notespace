...
test("P134 prepares a genuine bounded Delete through semantic encrypted storage without publication", async () => {
  ...
  assert.deepEqual(Object.keys(editor), ["preparePayloadEdit", "prepareMove", "prepareReorder", "prepareDelete", "prepareRestore"]); assert.equal(Object.isFrozen(editor), true);
...
test("P134 commits Delete once, reopens retained truth, and terminalises the full save surface", async () => {
  ...
  o.c.PocketStarlingRemoteEditShadow = Object.freeze({ createEditor: async (input) => { const editor = await real.createEditor(input); return Object.freeze({ preparePayloadEdit: editor.preparePayloadEdit, prepareMove: editor.prepareMove, prepareReorder: editor.prepareReorder, prepareDelete: async (value) => { const result = await editor.prepareDelete(value); if (result.outcome === "prepared") captured = result; return result; }, prepareRestore: editor.prepareRestore }); } });
  ...
  assert.deepEqual(Object.keys(t), ["savePayload", "saveMove", "saveReorder", "saveDelete", "saveRestore", "reconcileAmbiguous"]); assert.equal(Object.isFrozen(t), true); assert.equal(result.outcome, "committed"); assert.ok(captured);
  ...
  const after = f.r.calls.length; for (const call of [() => t.savePayload({ nodeId: "sibling", payload: {} }), () => t.saveMove({ nodeId: "sibling", fromIndex: 0, newParentId: "source", toIndex: 0 }), () => t.saveReorder({ nodeId: "sibling", fromIndex: 0, toIndex: 1 }), () => t.saveDelete({ nodeId: "sibling", fromIndex: 0 }), () => t.saveRestore({ nodeId: "branch", fromIndex: 0, newParentId: "source", toIndex: 0 }), () => t.reconcileAmbiguous()]) await assert.rejects(call(), (error) => error.code === "remote-save-state-invalid");
