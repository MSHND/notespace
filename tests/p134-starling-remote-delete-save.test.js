...
test("P134 commits Delete once, reopens retained truth, and terminalises the full save surface", async () => {
  ...
  assert.deepEqual(Object.keys(t), ["savePayload", "saveMove", "saveReorder", "saveDelete", "saveRestore", "saveInsert", "reconcileAmbiguous"]); assert.equal(Object.isFrozen(t), true); assert.equal(result.outcome, "committed"); assert.ok(captured);
