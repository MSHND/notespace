...
test("P210v source invariant keeps full reconstruction recovery-only and ordinary structural operations on patchProjection", () => {
  ...
  assert.doesNotMatch(runtime, /\bfunction render\(|\brender\s*\(/);
  ...
  assert.doesNotMatch(operations, /pane\.innerHTML|rebuildProjectionForRecovery|requestAnimationFrame\s*\([^)]*focusLine/);
  ...
  assert.ok((operations.match(/patchProjection\(/g) || []).length >= 8);
});
