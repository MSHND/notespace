...
test("P130 keeps existing payload move and reorder behaviour unchanged", async () => {
  ...
  assert.deepEqual(Object.keys(api), ["createBase", "insert", "editPayload", "move", "deleteBranch", "restoreBranch", "reorder", "compose", "diagnostics", "semanticTransitionBinding"]);
  assert.equal(Object.isFrozen(api), true);
  for (const method of ["createBase", "editPayload", "move", "deleteBranch", "restoreBranch", "reorder", "compose", "diagnostics"]) assert.equal(typeof api[method], "function");
});
