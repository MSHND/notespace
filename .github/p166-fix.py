...
'''      }
      const actualRevision''', "preserve ownership collision ordering")
s = once(s,
...
'''    const account = await authoriseObjectHead(context, request.syncedPocketId);
    return withPocketAuthorityLock(request.syncedPocketId, async () => {
      const authority = await transact("readonly", (transaction) =>''', "head authenticate before fence")
write(p, s)

# P032 exact route/API contract now includes the narrow authority client.
...
# P034 records/methods extend exactly; unrelated store/transaction surfaces remain unchanged.
...
'''    "conditionalUpload",
    ...
    "listEnvelopes",''', "p034 core methods")
write(p, s)

# P047 knows the additional strict collection; store public surface remains one transact method.
...
# Runtime PostgreSQL fixture: third schema marker/collection and advisory lock SQL outside transactions.
...
'''    versions: [{ schema_name: "pocket-sync-store", schema_version: 1 }, { schema_name: "pocket-sync-object-head-store", schema_version: 1 }, { schema_name: "pocket-sync-persistence-authority", schema_version: 1 }],''', "p049 schema versions")
s = once(s,
...
'''          if (sql.includes("pg_advisory_lock")) return { rows: [{ locked: null }], rowCount: 1 };
          ...
          if (!began) throw new Error("query outside transaction");
          const rowKey = key(values[0], values[1]);''', "p049 advisory queries")
s = once(s,
...
'''    assert.equal(calls[2], source("sync-service/migrations/002-pocket-sync-object-head-store.sql"));
    ...
    assert.equal(calls[4], "verify");''', "p049 migration order")
write(p, s)

# P118 owns migration/schema adoption proof and now includes P166's third additive migration.
...
'''const SECOND_MIGRATION = path.join(ROOT, "sync-service/migrations/002-pocket-sync-object-head-store.sql");
const THIRD_MIGRATION = path.join(ROOT, "sync-service/migrations/003-pocket-sync-persistence-authority.sql");''', "p118 third migration const")
s = s.replace("'recoveryCeremonies','keyOperations'))", "'recoveryCeremonies','keyOperations','persistenceAuthorities'))")
s = once(s,
...
'''    fs.readFileSync(SECOND_MIGRATION, "utf8"),
    ...
    "verify",''', "p118 migration expected")
s = once(s,
'''  for (const failAt of [1, 2]) await t.test(`apply ${failAt}`, async () => {''',
'''  for (const failAt of [1, 2, 3]) await t.test(`apply ${failAt}`, async () => {''', "p118 failure range")
s = once(s,
...
'''    const expected = [fs.readFileSync(FIRST_MIGRATION, "utf8"), fs.readFileSync(SECOND_MIGRATION, "utf8"), fs.readFileSync(THIRD_MIGRATION, "utf8")];
    assert.deepEqual(dependencies.calls, expected.slice(0, failAt));''', "p118 failure expected")
s = once(s,
'''  const second = fs.readFileSync(SECOND_MIGRATION, "utf8");''',
'''  const second = fs.readFileSync(SECOND_MIGRATION, "utf8");
  const third = fs.readFileSync(THIRD_MIGRATION, "utf8");''', "p118 migration read")
s = once(s,
...
'''  assert.doesNotMatch(second, /INSERT INTO public\.pocket_sync_(objects|heads)|UPDATE public\.pocket_sync_|DELETE FROM public\.pocket_sync_/i);
  ...
  assert.doesNotMatch(third, /UPDATE\s+public\.pocket_sync_records[\s\S]*encryptedRecord/i);''', "p118 migration semantics")
s = once(s,
...
# P119's deliberately tiny custom store retains its exact public surface but gains the private
# fence capability and the one authority row needed by actual Head CAS.
...
'''  const transact = async function transact(_mode, fn) { return fn(Object.freeze({ async get(collection,key) { reads.push([collection,key]); return rows.get(`${collection}\\0${key}`) || null; }, async insert(){},async replace(){},async remove(){} })); };
  ...
  const generic = Object.freeze({ transact });''', "p119 private lock")
write(p, s)
