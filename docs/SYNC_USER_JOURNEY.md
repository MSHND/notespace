...
This is the approved human journey for a future Synced Pocket. P027-P038 do not add these screens or turn sync on. There is no production account, passkey UI, emergency-recovery UI, device transfer or deployed remote service yet. P038 only aligns the unloaded same-origin browser client with the dormant key/recovery service core. It changes no approved copy.
...
P027-P038 add no production Turn on sync button, sign-in screen, account session, biometric request, emergency recovery package, device transfer, deployed backend, background sync, live synced owner or second Sync button. P038 can issue key/recovery requests only when a future caller explicitly injects and invokes its unloaded transport; nothing invokes it today. P034-P037 remain an undeployed CommonJS state machine, not a running service. The dormant boundaries are documented in [SYNC_REMOTE_CLIENT.md](SYNC_REMOTE_CLIENT.md), [SYNC_KEY_RECOVERY_REMOTE_CLIENT.md](SYNC_KEY_RECOVERY_REMOTE_CLIENT.md), [SYNC_SERVICE_CORE.md](SYNC_SERVICE_CORE.md) and [SYNC_KEY_RECOVERY_SERVICE.md](SYNC_KEY_RECOVERY_SERVICE.md); concrete orchestration, UI, HTTP/cookie/database adapters and owner integration still require review and tests.
