...
P027-P049 add no production Turn on sync button, sign-in screen, account session, biometric request, emergency-recovery UI, device transfer, deployed backend, background sync or second Sync button. The browser Sync foundations are loaded but inert, and nothing creates or invokes the activation runtime during startup. It neither opens IndexedDB nor begins a passkey ceremony until a future user-gesture caller explicitly invokes it with injected services. P039 can build the initial local-only recovery package and P041 can replace it after dormant recovery only when a future caller explicitly invokes their seams; P045 can compose the P039 path into a live owner only when explicitly called. P049 adds a separately invoked local HTTPS composition of the service layers, not a deployed backend. The dormant boundaries are documented in [SYNC_ACTIVATION_ORCHESTRATION.md](SYNC_ACTIVATION_ORCHESTRATION.md), [SYNC_EMERGENCY_RECOVERY_ORCHESTRATION.md](SYNC_EMERGENCY_RECOVERY_ORCHESTRATION.md), [SYNC_REMOTE_CLIENT.md](SYNC_REMOTE_CLIENT.md), [SYNC_KEY_RECOVERY_REMOTE_CLIENT.md](SYNC_KEY_RECOVERY_REMOTE_CLIENT.md), [SYNC_SERVICE_CORE.md](SYNC_SERVICE_CORE.md) and [SYNC_KEY_RECOVERY_SERVICE.md](SYNC_KEY_RECOVERY_SERVICE.md); UI and public deployment remain unimplemented.
