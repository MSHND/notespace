...
This is the approved human journey for a future Synced Pocket. P027-P045 do not add these screens or turn sync on. There is no production account, passkey UI, emergency-recovery UI, device transfer or deployed remote service yet. P039/P040 prove activation ordering and authority transition through injected dormant seams. P041 proves emergency recovery up to a durable non-authoritative state, including safe pause/resume until the replacement recovery copy is stored. P045 composes those foundations into an inert browser runtime for a future explicit action, while remote services remain injected. It changes no approved copy.
...
P027-P045 add no production Turn on sync button, sign-in screen, account session, biometric request, emergency-recovery UI, device transfer, deployed backend, background sync or second Sync button. P045 loads inert client-side foundations and exposes a browser activation runtime, but nothing creates or invokes it during startup. It neither opens IndexedDB nor begins a passkey ceremony until a future user-gesture caller explicitly invokes it with injected services. P039 can build the initial local-only recovery package and P041 can replace it after dormant recovery only when a future caller explicitly invokes their seams; P045 can compose the P039 path into a live owner only when explicitly called. P034-P037 remain an undeployed CommonJS state machine, not a running service. The dormant boundaries are documented in [SYNC_ACTIVATION_ORCHESTRATION.md](SYNC_ACTIVATION_ORCHESTRATION.md), [SYNC_EMERGENCY_RECOVERY_ORCHESTRATION.md](SYNC_EMERGENCY_RECOVERY_ORCHESTRATION.md), [SYNC_REMOTE_CLIENT.md](SYNC_REMOTE_CLIENT.md), [SYNC_KEY_RECOVERY_REMOTE_CLIENT.md](SYNC_KEY_RECOVERY_REMOTE_CLIENT.md), [SYNC_SERVICE_CORE.md](SYNC_SERVICE_CORE.md) and [SYNC_KEY_RECOVERY_SERVICE.md](SYNC_KEY_RECOVERY_SERVICE.md); UI, HTTP/cookie/database adapters and live owner integration still require review and tests.
