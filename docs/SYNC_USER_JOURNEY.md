...
This is the approved human journey for a future Synced Pocket. P027-P039 do not add these screens or turn sync on. There is no production account, passkey UI, emergency-recovery UI, device transfer or deployed remote service yet. P039 proves the complete activation ordering through injected dormant seams, including safe pause/resume until the recovery copy is stored. It changes no approved copy.
...
P027-P039 add no production Turn on sync button, sign-in screen, account session, biometric request, emergency-recovery UI, device transfer, deployed backend, background sync, live synced owner or second Sync button. P039 can build a local-only recovery package only when a future caller explicitly injects and invokes its unloaded seams; nothing invokes it today. P034-P037 remain an undeployed CommonJS state machine, not a running service. The dormant boundaries are documented in [SYNC_ACTIVATION_ORCHESTRATION.md](SYNC_ACTIVATION_ORCHESTRATION.md), [SYNC_REMOTE_CLIENT.md](SYNC_REMOTE_CLIENT.md), [SYNC_KEY_RECOVERY_REMOTE_CLIENT.md](SYNC_KEY_RECOVERY_REMOTE_CLIENT.md), [SYNC_SERVICE_CORE.md](SYNC_SERVICE_CORE.md) and [SYNC_KEY_RECOVERY_SERVICE.md](SYNC_KEY_RECOVERY_SERVICE.md); UI, HTTP/cookie/database adapters and live owner integration still require review and tests.
