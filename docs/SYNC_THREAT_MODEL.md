...
| Excessive AES-GCM key use | Raises random-nonce collision and usage risk | P029 declares a `2^31` ceiling; P030 schema 4 persists monotonic counters per long-lived master and device wrapping key below it | Per-device counts cannot prove a global cross-device total; remote/account enforcement and rotation remain required |
