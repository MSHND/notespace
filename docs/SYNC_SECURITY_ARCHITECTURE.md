...
P032/P038 implement the dormant request boundary beneath P031 and P028. One caller-supplied same-origin absolute-path root receives fifteen locked POST-only JSON routes with same-origin browser credentials, no-store caching, rejected redirects, no referrer and bounded JSON responses. The browser session is future server-owned cookie state; the client stores no bearer token or session, retries nothing automatically and accepts only exact versioned response shapes. P038 reuses P031 for recovery passkey validation and P029 for encrypted envelopes. See [Synced Pocket remote client](SYNC_REMOTE_CLIENT.md).
...
P048 implements the real but undeployed server WebAuthn verifier adapter, and P049 supplies a local-only HTTPS composition. The aligned client/core still do not implement a recovery-proof algorithm, selected public deployment, rate limiting, device transfer or deletion. The recovery-proof trust design is a reviewed P050 blocker: Pocket has not chosen whether a service database compromise may forge recovery authorisation. P039 and P041 implement local cryptographic/package orchestration only behind dormant injected boundaries. No synced owner exists until an explicit future adoption flow runs.
