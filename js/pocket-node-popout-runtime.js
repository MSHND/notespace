...
(function initialisePocketNodePopoutRuntime(global) {
  ...
  function initialise(initialPayload, environment) {
    ...
    var dirty = false, editGeneration = 0, allowedToClose = false, saveInFlight = false, saveGeneration = null, closeAfterSaveGeneration = null;
    ...
    function markMutation() { if (readOnly) return false; editGeneration += 1; closeAfterSaveGeneration = null; setDirty(true); return true; }
    ...
    function settleSaveResult(result, closeAfter, generation) { var queuedClose = closeAfterSaveGeneration === generation && editGeneration === generation; saveInFlight = false; saveGeneration = null; closeAfterSaveGeneration = null; return handleSaveResult(result, closeAfter || queuedClose, generation); }
    function save(closeAfter) { if (readOnly || saveInFlight) return false; hideUnsavedDialog(); if (!hasCompleteSaveContext()) { handleSaveResult({ok:false,reason:"missing-source-identity"},closeAfter,editGeneration); return false; } setSaveState("saving…",""); saveInFlight = true; var generation = editGeneration; saveGeneration = generation; var outgoing = buildPayload(); try { var target = openerPopoutWindow(); if (target && typeof target.applyAndSaveFromOwnedPopup === "function") { Promise.resolve(target.applyAndSaveFromOwnedPopup(ownerToken,popupToken,outgoing,window)).then(function (result) { settleSaveResult(result,closeAfter,generation); }, function (error) { saveInFlight=false; saveGeneration=null; closeAfterSaveGeneration=null; console.error(error); setDirty(true); setSaveState("Truth-file write failed — not saved","failed"); }); return true; } } catch (error) { console.error(error); } saveInFlight=false; saveGeneration=null; closeAfterSaveGeneration=null; handleSaveResult({ok:false,reason:"popup-session-changed"},closeAfter,generation); return false; }
    function closeSafely() { if (readOnly || !dirty) { allowedToClose=true; window.close(); return; } if (saveInFlight && saveGeneration === editGeneration) { closeAfterSaveGeneration = saveGeneration; return; } showUnsavedDialog(); }
