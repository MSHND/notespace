...
(function initialisePocketNodePopoutTemplate(global) {
  ...
  function surfaceScriptUrls() {
    const authority = global.PocketSurfaceDependencies;
    if (!authority || typeof authority.scriptsFor !== "function"
        || typeof global.URL !== "function" || !global.location || typeof global.location.href !== "string") {
      throw new Error("PocketNodePopoutTemplate.render requires the PE surface dependency contract.");
    }
    let scripts;
    try { scripts = authority.scriptsFor("pe"); } catch (_error) {
      throw new Error("PocketNodePopoutTemplate.render requires the PE surface dependency contract.");
    }
    if (!Array.isArray(scripts) || scripts.length === 0) throw new Error("PocketNodePopoutTemplate.render requires PE surface scripts.");
    const base = new global.URL(global.location.href);
    return scripts.map(function (asset, index) {
      let resolved;
      try { resolved = new global.URL(asset, base); } catch (_error) {
        throw new Error("PocketNodePopoutTemplate.render requires safe PE surface scripts.");
      }
      if (resolved.origin !== base.origin || resolved.search || resolved.hash) {
        throw new Error("PocketNodePopoutTemplate.render requires same-origin PE surface scripts.");
      }
      return safeAsset(resolved.pathname, `PE script ${index + 1}`);
    });
  }
  function render(payload, helpers) {
    helpers = helpers || {};
    const htmlEscape = typeof helpers.htmlEscape === "function" ? helpers.htmlEscape : fallbackHtmlEscape;
    const scriptAssetUrls = surfaceScriptUrls();
